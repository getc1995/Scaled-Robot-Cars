...
Mat3b canvas;
string winName = "Manual Control 2";
...
string button_v_text("ACC");
string button_s_text("LK");
...
{
    ros::init(argc, argv, "remote_gui2");
    ros::NodeHandle nh;
    servo_pub = nh.advertise<std_msgs::UInt16>("servo2", 1);
    motor_pub = nh.advertise<std_msgs::Float32>("Motor_command2", 1);
    control_pub = nh.advertise<um_acc::CentralSignal>("Control_command2", 1);

    ros::Subscriber IMU_sub = nh.subscribe("IMU_data2", 2, IMUCallback);
    ros::Subscriber motor_sub = nh.subscribe("Motor_data2", 2, motorCallback);
