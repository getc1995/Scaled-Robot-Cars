...
int main(int argc, char** argv)
{
  ...
  control_pub = nh.advertise<std_msgs::String>("Control_interface", 1);
...
void TeleopCar::keyLoop()
{
  ...
  for(;;) {
      ...
      switch(c)	{
      	...
      	case KEYCODE_R:
      	  ROS_DEBUG("RIGHT");
      	  steer = min(1600, steer+STEER_STEP);
      	  steerDirty = true;
      	  break;
      	case KEYCODE_U:
      	  ROS_DEBUG("UP");
        ...
        case ' ':
          ...
          ros::spinOnce();
          loop_rate.sleep();
          std::cout << controlMsg.data << std::endl;
          current = 0;
          motorDirty = false;
          break;
        ...
        case 'c':
          ROS_DEBUG("CC");
          char currentStr[50];
          sprintf(currentStr, "CC %0.4lf", 0.2);
          controlMsg.data = currentStr;
          control_pub.publish(controlMsg);
          ros::spinOnce();
          loop_rate.sleep();
