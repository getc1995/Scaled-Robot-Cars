...
int main(int argc, char **argv)
{
    ...
    ros::Publisher servo_pub = n.advertise<std_msgs::UInt16>("servo1", 1);
    ros::Publisher motor_pub = n.advertise<std_msgs::Float32>("Motor_command1", 1);
    ...
    ros::Subscriber command_sub = n.subscribe("command", 2, commandCallback);
    ...
    while(ros::ok())
    {
        ...
        if (go == true){
            ...
            int steer = int(- 6*msg.y - 5*(msg.phi*180/3.14159));
            steer = min(steer, 200);
            steer = max(steer, -200);
            steer = steer + 1409;
            servoMsg.data = steer;
            ...
            motorMsg.data = 1.8 + (0.6 - u)*2;
            motor_pub.publish(motorMsg);
            cout << "#### command I: " << motorMsg.data << "Steer: " << steer << endl;
        }
